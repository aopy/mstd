...
1. **Two Directions/Two Neurons**: The network is trained to recognize two directions of a moving bar:
   left-to-right and right-to-left. There are two neurons, each expected to become selective (i.e. spike) to one
...
def create_moving_bars_stimulus_with_delay_and_labels2(batch_size, width, height, bar_width, time_step, synaptic_delay=1, direction=""):
    global right_count, left_count, direction_choice, le, ri

    # moving bars stimulus with synaptic delay and labels
    current_stimulus = torch.zeros(batch_size, height, width)
    delayed_stimulus = torch.zeros(batch_size, height, width)

    # Calculate probability to favor the less frequently chosen direction
    if right_count + left_count > 0:  # Check to avoid division by zero
        p_left = right_count / (right_count + left_count)  # Probability favoring 'left' if 'right' is chosen more often
    else:
        p_left = 0.5  # Equal probability for the first choice
    ...
    if time_step == 0:
        if direction:
            ...
            # Choose direction based on the dynamically adjusted probabilities
            direction_choice = 'left' if random.random() < p_left else 'right'
            # Update counts based on chosen direction
            if direction_choice == 'right':
                right_count += 1
            else:
                left_count += 1
...
if __name__ == '__main__':
    ...
    S, batch_size, width, height, bar_width = 5000, 1, 10, 10, 1  # 40 39
    ...
    membrane_potentials = {direction: torch.zeros(2, 13) for direction in test_stimuli}
        ...
        for i, direction in enumerate(test_stimuli):
            for neuron_index in range(N_out):
                ...
                for t in spike_times:
                    axs[i].axvline(x=t, color='k', linestyle=':',
                                   label='Spike' if t == spike_times[0] and neuron_index == 0 else "")
