...
Key Features:
1. **Two Directions/Three Neurons**: The network is trained to recognize two directions of a moving bar:
   left-to-right and right-to-left. There are three neurons, with two neurons expected to become selective to one
   direction and one neuron to the other direction after training.
...
class MySpikingNetwork(MemoryModule):
    def __init__(self, input_size):
        ...
        self.fc = nn.Linear(input_size, 3, bias=False)
...
if __name__ == '__main__':
    ...
    # nn.init.uniform_(net.fc.weight.data, 0.2, 0.3)
    nn.init.constant_(net.fc.weight.data, 0.4)
    ...
    membrane_potentials = {direction: torch.zeros(3, 13) for direction in test_stimuli}
    ...
    if test_stimuli:
        ...
        colors = ['blue', 'orange', 'green']  # Colors for Neuron 1 and Neuron 2
