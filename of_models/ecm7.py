...
class EventDataset(Dataset):
    def __init__(self, file_path, calibration_file, height=720, width=1280, camera_index=3, chunk_size=100000,
        ...
        # Select the right camera (index 3 in your provided calibration data)
        intrinsics = calibration_data['intrinsics'][camera_index]['intrinsics']
    ...
    def preprocess_events(self, events, center_x=640, center_y=360, rf_width=14, rf_height=11):
        ...
        # Adjust the coordinates based on calibration data to correct for angular resolution differences
        events[:, 0] = (events[:, 0] - self.cx) / self.fx * self.width + self.cx
        events[:, 1] = (events[:, 1] - self.cy) / self.fy * self.height + self.cy
        ...
        for event in events:
            ...
            if x_min <= x < x_max and y_min <= y < y_max:
                x_rf = x - x_min
                y_rf = y - y_min
                if 0 <= x_rf < rf_width and 0 <= y_rf < rf_height:  # Ensure indices are within bounds
                    if polarity == 1:
...
if __name__ == '__main__':
    ...
    dataset = EventDataset(file_path, calibration_file='data/camera-calibrationB.json',
                           max_events=max_events, temporal_window=temporal_window, delay=20e3)
